...
AOCSmode.time = cell();
AOCSmode.time(1).entries = [-1; 50];         // each time need a start time and end time.
AOCSmode.time(2).entries = [500; 90];



AOCSmode.Target = cell();
AOCSmode.Target(1).entries = [0; 0; 1];
AOCSmode.Target(2).entries = [0; 0; -1];
